...
export type LeadFormKind = 'client' | 'lawyer' | 'law-firm' | 'notary';

const KIND_LABELS: Record<LeadFormKind, string> = {
	client: 'Заявка клієнта',
	lawyer: 'Заявка юриста',
	'law-firm': 'Заявка юридичної фірми',
	notary: 'Заявка нотаріуса',
};

/** Inline lead-capture form used on the /for-* landing pages. Sends to the Legal Telegram contact channel. */
@Component({
...
export class LeadFormComponent {
	...
	protected async submit(form: NgForm): Promise<void> {
		...
		const lines = [
			`Legal lead: ${KIND_LABELS[this.kind()]}`,
			`Ім'я: ${this.name()}`,
